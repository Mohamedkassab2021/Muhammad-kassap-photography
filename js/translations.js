const translations = {
  en: {
    siteName: "Muhammad kassap",
    siteSubName: "Photography",
    home: "Home",
    information: "Pricing and packages",
    portfolio: "Portfolio",
    selectLanguage: "Select a language",
    english: "English",
    arabic: "Arabic",
    pic: "Wedding Alliance Photography",
    paragraph: "We try to make your day happy with our photography team",
    hy: "Welcome",
    info:"Mohamed Kassab Photography",
    info1:"Experience of not less than 6 years in the field of wedding and event photography",
    info2:"And speed in delivery and maintain high image quality with the best print quality for albums and tablets and a lot of exclusive gifts at reasonable prices for all segments of society We wish you a nice 😍 day",
    price:"Pricing and packages",
    portfolio:"Portfolio",
    story:"story",
    pack:"Portfolio & Services",
    all:"All",
    wedding:"Wedding",
    before:"Engagement",
    port:"Katb Ketab",
    inf:"Information",
    dis:"Clear Prices & Complete Packages",
    prices:"Are you looking for an experienced destination photographer for affordable prices? You found me!My pricing and packages are designed to appeal to everyone.",
    prices2:"We provide all services at the best prices to the maximum possible!",
    sale:"Just For = 1200 Eg",
    salee:"Just For = 2300 Eg",
    saleee:"Just For = 4000 Eg",
    saleeee:"Just For = 7000 Eg",
    pacename:"Diamond Package ",
    pacenamee:"Gold Package ",
    pacenameee:"package Full Day ",
    pacenameeee:"Vip package",
    packinf1:"Photo Seasion only",
    packinf:"Photo Seasion",
    packinff:" Editing Of All Photos ",
    packinfff:" Delvery Your Photo With Usb Flash ",
    packinffff:"If you are outside Menoufia, 400 pounds will be added ",
    packinfffff:"Party",
    packinffffff:" Album & Portrait",
    packinfffffff:" Preparations",
    packinffffffff:" Crutch Or Katb Ketab",
    packinfffffffff:" Promo Video",
    packinffffffffff:" 2 Photographers",
    serv:"Extra Services",
    serve:"If you like a specific type of photography you can contact us",
    se:"Second photographer",
    wed:"Wedding album",
    portraitt:"Portrait photo seasion",
    com:"Commercial",
    s:"1000 Eg",
    w:"From 800 Eg",
    p:"From 350 Eg",
    c:"Contact Us for Details",





  },
  ar: {
    siteName: "محمد كساب",
    siteSubName: "فوتوغرافى",
    home: "الصفحة الرئيسية",
    information: "التسعير و الباكدجات",
    portfolio: "اعمالنا",
    selectLanguage: "إختر لغة",
    english: "الانجليزية",
    arabic: "العربية",
    pic: "تصوير حفل الزفاف",
    paragraph: "نحن نحاول أن نجعل يومك سعيدًا مع فريق التصوير لدينا",
    hy: "اهلا بك",
    info:"محمد كساب فوتوغرافى",
    info1:"خبرة لا تقل عن 6 سنين فى مجال تصوير الافراح والمناسبات",
    info2:"وسرعة فى التسليم و الحفاظ على جودة الصور عالية مع افضل جودة طباعة للالبومات و التابلوهات و الكثير من الهدايا الحصرية باسعار مناسبة لجميع فئات المجتمع نتمنى لكم يوما لطيفا 😍",
    price:"الأسعار والحزم",
    portfolio:"الصور والخدمات",
    story:"اللحظات",
    pack:"الصور والخدمات",
    all:"الكل",
    wedding:"الزفاف",
    before:"الخطوبه",
    port:"كتب الكتاب",
    inf:"التفاصيل",
    dis:"كل المعلومات والحزم الكاملة",
    prices:"هل تبحث عن مصور وجهة من ذوي الخبرة بأسعار معقولة؟ لقد وجدتني! تم تصميم أسعاري وحزمي لجذب الجميع. ",
    prices2:"نحن نقدم جميع الخدمات بأفضل الأسعار إلى أقصى حد ممكن!",
    sale:"فقط بسعر = 1200 ج",
    salee:"فقط بسعر = 2300 ج",
    saleee:"فقط بسعر = 4000 ج",
    saleeee:"فقط بسعر = 7000 ج",
    pacename:"الباقة الماسية ",
    pacenamee:"الباقة الذهبية ",
    pacenameee:"حزمة يوم كامل ",
    pacenameeee:"باقة كبار الشخصيات",
    packinf1:"حفله تصوير فقط",
    packinf:"حفله تصوير",
    packinff:" تعديل جميع الصور",
    packinfff:"USB توصيل كل صورك مع ",
    packinffff:"اذا كنت خارج المنوفية يتم اضافة 400 جنيه ",
    packinfffff:"حفله",
    packinffffff:" الألبوم والبورتريه",
    packinfffffff:"التجهيزات",
    packinffffffff:"كتب الكتاب",
    packinfffffffff:" فيديو ترويجي",
    packinffffffffff:" 2 المصورين",
    serv:"المزيد من الخدمات",
    serve:"إذا كنت تحب نوع معين من التصوير الفوتوغرافي يمكنك الاتصال بنا",
    se:"مصور ثاني",
    wed:"ألبوم الزفاف",
    portraitt:"حفل تصوير بورتيريه",
    com:"تجاري",
    s:"1000 جنيه",
    w:"تبدا من 800 جنيه",
    p:"تبدا من 350 جنيه",
    c:"تواصل معنا للتفاصيل",

  

  },
};

export default translations;
